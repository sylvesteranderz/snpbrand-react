...
const ProductDetail = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star
        key={i}
        className={`w-5 h-5 ${i < Math.floor(rating) ? 'text-primary-400 fill-current' : 'text-gray-300'
          }`}
      />
    ))
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <div className="flex items-center space-x-4">
              <span className="text-3xl font-bold text-price-500">{formatPrice(product.price)}
              </span>
