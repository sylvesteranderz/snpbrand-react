  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      if (formData.deliveryMethod === 'delivery') {
        ...
        if (!formData.campus) newErrors.campus = 'Please select a campus for pickup'
      }
    }
                ...
                >
                  ...
                  <div className="flex gap-4 mb-6">
                    <button
                      ...
                      className={`flex-1 py-3 px-4 border-2 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2 ${formData.deliveryMethod === 'delivery'
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 px-4 border-2 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2 ${formData.deliveryMethod === 'pickup'
                        ? 'border-primary-500 bg-primary-50 text-primary-700'
                        : 'border-gray-200 text-gray-600 hover:border-primary-300'
                        }`}
                    >
                    ...
                    {formData.deliveryMethod === 'delivery' ? (
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-5">
                        ...
                        <div className="space-y-3">
                          <label className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${formData.campus === 'legon' ? 'border-primary-500 bg-white shadow-sm' : 'border-gray-300 bg-white hover:bg-gray-50'}`}>
                            <input
                              type="radio"
                              name="campus"
                              value="legon"
                              checked={formData.campus === 'legon'}
                              onChange={() => setFormData({ ...formData, campus: 'legon', address: '', city: '' })}
                              className="w-4 h-4 text-primary-600 focus:ring-primary-500 border-gray-300"
                            />
                            <div className="ml-3 flex-1">
                              <span className="block text-sm font-medium text-gray-900">University of Ghana, Legon Campus</span>
                              <span className="block text-xs text-gray-500 mt-0.5">Pickup at main gate or agreed location on campus</span>
                            </div>
                          </label>
                          <label className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${formData.campus === 'knust' ? 'border-primary-500 bg-white shadow-sm' : 'border-gray-300 bg-white hover:bg-gray-50'}`}>
                            <input
                              type="radio"
                              name="campus"
                              value="knust"
                              checked={formData.campus === 'knust'}
                              onChange={() => setFormData({ ...formData, campus: 'knust', address: '', city: '' })}
                              className="w-4 h-4 text-primary-600 focus:ring-primary-500 border-gray-300"
                            />
                            <div className="ml-3 flex-1">
                              <span className="block text-sm font-medium text-gray-900">KNUST Campus, Kumasi</span>
                              <span className="block text-xs text-gray-500 mt-0.5">Pickup at Jubilee Mall or agreed location on campus</span>
                            </div>
                          </label>
                        </div>
                        {errors.campus && (
                          <p className="text-red-500 text-sm mt-2 font-medium">{errors.campus}</p>
                        )}
                      </div>
                    )}
                  </div>
