...
const CategorySection = ({ className = '' }: CategorySectionProps) => {
  ...
  const currentCategory = dynamicCategories.find(cat => cat.id === activeCategory)

  // Recalculate subcategory counts
  const currentSubcategories = activeCategory !== 'all'
    ? (subcategories[activeCategory as keyof typeof subcategories] || []).map(sub => ({
      ...sub,
      count: products.filter(p => p.category === activeCategory && p.subcategory === sub.id).length
    }))
    : []
        ...
        >
          {dynamicCategories.map((category) => (
            <button
              ...
              className={`flex items-center space-x-3 px-6 py-4 rounded-lg border-2 transition-all duration-300 ${activeCategory === category.id
        ...
        {currentSubcategories.length > 0 && (
          ...
          >
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeSubcategory === 'all'
            ...
            {currentSubcategories.map((subcategory) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeSubcategory === subcategory.id
                    ? 'bg-primary-500 text-white'
                    : 'bg-gray-100 text-gray-700 hover:bg-primary-100'
                  }`}
              >
